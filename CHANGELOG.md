...
- **LLM — thinking / chat template (llama-server):** When **Allow thinking** is off, each **`/v1/chat/completions`** request still sends **`chat_template_kwargs.enable_thinking=false`** (belt-and-suspenders with **`--reasoning-budget 0`**); refactored payload build in **`LlamaCppServerLLM.build_chat_completions_payload`**. Optional kwargs: **`chat_messages`** (replace default system+user), **`skip_chat_template_kwargs`** (bisection). **Settings → Advanced → LLM template diagnostic:** three toggles (bypass generic system line, omit **`chat_template_kwargs`**, show JSON request preview) + **Send Hello test prompt** for support / model bring-up. Same run calls **`LlamaCppServerLLM.fetch_rendered_prompt_via_apply_template`**: probes **`POST /apply-template`** then **`POST /v1/apply-template`** with **`messages`** + optional **`chat_template_kwargs`** aligned with completions; results under **Rendered prompt (post-Jinja)** (graceful message if both paths 404).
- **Config:** **`feedback.llm_diag_*`** keys in **`App/configs/defaults.yaml`** (persisted via **Save** like other Advanced options).
- **Chat / Help thinking-tag hardening:** Expanded model-output cleanup to remove leaked `<think>`, `<thinking>`, `<reasoning>`, and `<redacted_thinking>` markers even when blocks are empty, partial, or orphaned; Chat/Help bubble rendering now applies an additional defensive strip pass. LLM greeting cache stores only post-sanitized visible text, and Chat welcome auto-TTS speaks that same sanitized text (never raw tags).
- **Ask cross-mode memory default:** **`llm.inject_all_modes_in_chat`** factory default is now **Off** (new installs and missing-key fallback), so non-Ask LLM modes are not mirrored into Ask history unless explicitly enabled in Settings.
- **Advanced diagnostic UX (16 locales):** Diagnostic labels were shortened and clarified (`Skip instructions`, `Allow reasoning`, `Show raw request`, `Run test`) to avoid truncation in narrow windows; layout tightened (fixed label width + narrower On/Off dropdowns) and localized strings added for all shipped UI locales.

### Beta 0.29.4 (April 2026) — shipped
- **LLM sampling (Settings → Performance, llama-server):** User-tunable **`llm.temperature`**, **`top_p`**, **`top_k`**, **`min_p`**, **`repeat_penalty`**, **`presence_penalty`**, and **`frequency_penalty`** appear as dropdown rows after **Allow thinking** / **Thinking token budget**. Values persist in **`Data/Configs/config.yaml`** under **`llm:`** and are sent on **`/v1/chat/completions`** alongside **`temperature`** (`App/services/llm/llamacpp_server.py`, merged config via **`Orchestrator._llm_sampling_kwargs()`**). UI lists snap handwritten YAML values to the nearest preset. Changing them flags **Save & Restart** like other Performance LLM fields. Intended defaults reference **instruct-style** sampling (comments in **`App/configs/defaults.yaml`**). **Documentation:** **`README.md`** (Configuration), **`GETTING_STARTED.md`**, **`ARCHITECTURE.md`** (Performance + Version History).
- **STT in `mix` — fidelity path (Ask + Transcribe):** with **`audio.capture_mode: mix`**, **`answer`** (Alt+A / Chat / Continuous) and **`transcribe`** (Alt+T) use **single-pass Whisper on the full mixed waveform** (`Orchestrator._build_dual_mix_raw_text` returns `None` for those modes). This avoids **truncated phrase endings** that dual-track segmentation could drop. Dual-track / source-labelled STT remains in the codebase for other modes when that path is used; segment merging keeps **short edge segments** when dual-track runs.
- **Continuous Chat (beta):** Chat-tab **Continuous Chat** switch — listen until silence → **Ask** → optional **Pro** TTS auto-speak → re-arm mic. Same **Performance → STT source** as manual Ask. **Manual mode hotkeys** take over and **exit** Continuous; **sidebar avatar click** stops Continuous. **Idle guard:** low-energy turns are not discarded once the clip is long enough (~**0.7 s**+) so STT still runs. **`Alt+Q`** / cancel call **`reset_continuous_chat_listen_blockers`** (TTS cooldown + GUI pending); post-TTS anti-loopback uses **playback duration + tail** without double-counting **prepare** time.
- **Stop / UI desync:** **`Orchestrator.stop_recording()`** clears **`_is_recording`** then **`recorder.request_stop()`**. **`Esc` / `Alt+Q`** and **avatar** also force stop when status is **Listening** or **`_is_recording`** is true even if **`recorder.is_recording`** is already false (avoids stuck “Listening” with no stop).
- **Diagnostics:** optional **`audio.pipeline_debug: true`** in `Data/Configs/config.yaml`, or **Settings → Advanced → Debug mode** (also enables Full Console payload dumps). Emits **`[pipeline]`** (VAD, buffers, mix mic/loopback lengths, mic queue drops) and **`[continuous]`** (wait vs arm, cooldown extend, blocker reset) on logger **`perkysue`**. See **TROUBLESHOOTING.md**. Recorder picks up the flag on **Save & Restart** / rebuild (`build_audio_recorder(..., pipeline_debug=…)`).
- **Smart Focus (Pro):** each voice turn targets **`get_active_window`** → **`inject_text`** at hotkey/arm time; **Continuous** refreshes the HWND each listen segment. PerkySue-own window can **skip external paste** for Answer (Chat-only).
- **Post-update (see 0.29.3):** first launch after in-app update runs dependency probe; Windows **`system_only`** / **`mix`** includes **`pyaudiowpatch`** check with explicit logs on failure.
- **OmniVoice install/repair (Windows) stabilization:** Voice-tab install flow now handles TorchCodec + FFmpeg shared dependencies in a deterministic sequence and no longer fails the final verify step when OmniVoice returns `numpy.ndarray` audio instead of a `torch.Tensor`. Result: OmniVoice install completes cleanly on tested bundles, with and without Pro TTS usage afterward.
- **Voice repair UX hardening:** Voice repair paths now better distinguish lightweight engine retry vs full repair requirements, and full repair relaunch avoids competing app instances/hotkey collisions during `install.bat` recovery.
- **Plugin connector (internal beta):** Added/validated host-side connector path for the **Brainstorm** plugin prototype (roundtable/debate AI conversation workflow) through the `Data/Plugins/<id>/` extension mechanism. This remains an internal beta extension and is not required for core app operation.
...
- **Hotfix — post-update dependency checks for system audio capture:** Post-update runtime consistency probe now checks **`webrtcvad`** and conditionally checks **`pyaudiowpatch`** (**PyAudioWPatch**, WASAPI loopback) on Windows when `audio.capture_mode` is `system_only` or `mix`, preventing silent fallback to mic-only on machines missing loopback dependencies.
...
- **Cross-mode conversation memory (Pro):** New setting **Inject all modes in chat** (`llm.inject_all_modes_in_chat`), now **enabled by default**. When enabled, LLM exchanges from `Alt+M/I/P/L/C/D/X/S/G/V/B/N` are mirrored into Chat history so follow-ups work across modes (e.g., iterative email rewrites).
- **Performance defaults (new installs):** `llm.max_input_tokens` → **4096**, `llm.max_output_tokens` → **4096**, `llm.answer_context_keep` → **2**, `llm.inject_all_modes_in_chat` → **true**.
- **Default voice sample path resolution:** Built-in Default skin now supports locale-aware reference layout (`App/Skin/Default/<Locale>/audios/voice_sample/voice_sample.wav`) while preserving legacy fallback (`App/Skin/Default/audios/voice_sample/voice_sample.wav`).
- **Mix capture source separation for Ask context:** Dual-track handling in mix mode preserves microphone/system chronology and uses source-aware labels for LLM context while keeping plain text output for pure transcription mode.
- **Max listening duration (UI):** Performance **Max listening duration (s)** now offers **60 → 900 s** (15 min), not only up to 240 s. Values outside the list in `config.yaml` snap to the nearest option (60–900). Longer sessions increase **RAM** during capture + single-pass STT; use **`stt.chunk_*`** in config for very long clips if needed.
- **STT audio source (`audio.capture_mode`):** **Microphone only**, **System audio only** (WASAPI loopback via PyAudioWPatch on Windows), or **System + Mic mix** — selectable under **Settings → Performance** and wired through `build_audio_recorder` / `Orchestrator` so dictation and Ask use the intended capture path.
- **Llama-server / clipboard injection UX:** When the **local LLM HTTP connection drops** (e.g. process crash, **WinError 10054** / connection reset / remote closed) during or after a **paste-heavy** workflow, user-facing **document injection** errors now distinguish **transport / server gone** from **HTTP 400 / context-too-large** — avoids wrongly telling users to raise **Max input** when the issue was the **server connection**, not prompt size (`Orchestrator._document_injection_llm_error_message` + `header_alerts` `document_injection.llm_error_connection`).
- **Microphone selection (Windows default changes):** **Settings → Performance → Microphone input** — dropdown to pick the **sounddevice** input explicitly (including **Windows default** label and **orphan** `#index` rows when the saved device is not in the current list). Audio recorder rebuilds when Performance audio fields change. **Follow-up:** default Windows mic changes may still not always propagate as expected without an explicit pick or refresh — behavior under review.
- **Voice activity (silence auto-stop):** **`audio.vad_sensitivity`** presets **`quiet_room` | `normal` | `noisy`** (replaces factory **`vad_aggressiveness`** in `defaults.yaml`; legacy **0–3** int still accepted). **3-chunk (~300 ms) majority** smoothing on VAD decisions so isolated clicks / transients less often reset the silence timer. **Settings → Performance → Voice detection** dropdown + strings (16 locales). Resolver + ring buffer in **`App/utils/audio.py`**; orchestrator passes `vad_sensitivity` with fallback to `vad_aggressiveness`.

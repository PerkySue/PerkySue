...
Ask memory (Pro): **Remember last Q/A** = 2, 3, or 4 (default **2**). Cross-mode continuity (Pro): **Inject all modes in chat** = Off/On (default **Off**), which can append non-Ask LLM mode exchanges into shared Ask history for follow-up.
New-install defaults (0.29.2 closure): `max_input_tokens=4096`, `n_ctx=4096`, `max_output_tokens=4096`, `answer_context_keep=2`, `inject_all_modes_in_chat=false`.

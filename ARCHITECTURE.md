...
│  Data/Plugins/<id>/             ← Optional SPI extensions (manifest + code) │
...
│    │   ├── voice_modes.yaml       ← Voice / <PS_PAYLOAD> overlays per mode (merge Data/Configs/voice_modes.yaml) │
...
### Optional SPI extensions (`Data/Plugins/`)

PerkySue peut charger des **extensions optionnelles** fournies comme dossiers sous **`Data/Plugins/<plugin_id>/`** (données utilisateur, **hors** mise à jour de `App/`). Chaque extension expose un **`manifest.yaml`** (identifiant, version, `enabled`, éventuel gating Pro) et un module d’entrée ; l’hôte les découvre via **`App/utils/plugin_host.py`** (`PluginHostContext`, `load_plugin_module`, validation d’id).

- **Rôle de l’hôte :** construire un contexte **`PluginHostContext`** avec des hooks cadrés (`invoke_llm`, `list_skins`, `get_orchestrator`, etc.) pour que l’extension **n’importe pas** directement les modules internes `App/…`.
- **GUI :** l’app peut enregistrer onglets ou entrées de barre latérale lorsque `check_enabled` réussit et que le manifeste l’indique (même mécanisme que pour les autres fonctionnalités conditionnelles).
- **Documentation fonctionnelle** d’une extension livrée avec le produit : fichiers Markdown **dans le dossier de l’extension** (ou équivalent), sans exposer de chemins privés hors dépôt dans les docs publiques.
- **i18n (GUI d’extension) :** les libellés peuvent réutiliser le même chargeur que l’app — clés sous **`plugins.<plugin_id>.*`** dans **`App/configs/strings/<lang>.yaml`** (16 locales) et **`s()`** dans `App/utils/strings.py` (ex. panneau optionnel **Brainstorm** : `plugins.brainstorm.*`). Pas de second chargeur dans le plugin.

Les extensions sont **ajoutées par l’utilisateur ou le packager** ; cette section ne liste pas d’identifiants nominatifs d’extensions internes de développement.
...
- **Implementation:** `App/utils/audio.py` — **`AudioRecorder`**, **`SystemLoopbackRecorder`**, **`MixedDualInputRecorder`**; factory **`build_audio_recorder`**; **`Orchestrator._rebuild_audio_recorder_from_config`** passes merged **`audio`** fields (sample rate, silence timeout, capture mode, gains, **`vad_sensitivity`** with fallback to legacy **`vad_aggressiveness`**) and **`pipeline_debug`** from **`Orchestrator._audio_pipeline_debug()`** (`feedback.debug_mode` **or** **`audio.pipeline_debug`** in YAML).
- **Mix + Whisper routing (0.29.4):** When **`capture_mode`** is **`mix`**, **`Orchestrator._build_dual_mix_raw_text`** intentionally returns **`None`** for modes **`answer`** and **`transcribe`** so STT runs **once** on the **full mixed** float32 buffer (same path as **`mic_only`** for Whisper). This prioritizes **dictation fidelity** (no dropped tail after short pauses) over mic/system-labelled dual transcription for those two entry points. Other LLM modes may still take the dual-track branch when enabled.
...
#### Ask (Alt+A), Continuous Chat, Smart Focus, and `<PS_PAYLOAD>` (Beta 0.29.4+)

- **Continuous Chat worker:** `Orchestrator._continuous_chat_worker` — waits while TTS **`is_speaking()`**, **`_is_processing`**, **`_is_recording`**, **`_continuous_gui_tts_pending`**, or **`_continuous_tts_cooldown_until`**; then **`stop_voice_output()`**, sets **`_target_window = get_active_window()`**, and calls **`_record_and_process(..., continuous_chat_turn=True, suppress_no_speech_feedback=True)`**. **`_on_escape_hotkey`** disables Continuous before delegating to the widget stop path. **`reset_continuous_chat_listen_blockers`** clears cooldown + pending flag (logged when **`pipeline_debug`**).
- **Where it runs:** `_record_and_process` → `_process_audio_impl` (mode **`answer`**). Chat UI passes **`from_chat_ui=True`** for mic / Send / Continuous turns.
- **External injection vs Chat-only:** `skip_injection_answer_into_app` in `orchestrator.py` is true when **`(from_chat_ui and not continuous_chat_turn)`** — so a normal **Chat** turn (typed or single mic without Continuous) does **not** call **`inject_text`** into another app (result stays in the Chat tab; **Alt+T** remains the path into the in-app type-in). **`continuous_chat_turn=True`** (Continuous Chat worker) **clears that skip**: each completed turn can **paste into the window that had focus when the listening arm started** (`get_active_window()` refreshed at the start of the continuous loop iteration before `_record_and_process`). Pro-only external paste for Answer still applies (`is_effective_pro()`); Free stays in-app.
- **Voice payload (Pro TTS + Voice tab “payload layout”):** When **`tts.voice_payload_enabled`** and the **`answer`** overlay in **`App/configs/voice_modes.yaml`** are active, the system prompt gains the **two-part** voice format. The model may wrap pasteable text in **`<PS_PAYLOAD>…</PS_PAYLOAD>`** (or **`<PS_SUBJECT>`** / **`<PS_BODY>`** — parsed by **`App/utils/voice_payload.py`** → **`split_voice_payload_reply`**). **`answer_voice_skip_external`:** if voice-payload rules apply and the reply has **no** usable payload, PerkySue **does not** inject into the external app (Chat + TTS only). If a payload is present, **`injectable_plain`** is what **`inject_text`** receives for that turn.
- **Prompt maintenance:** The **`answer`** block in **`voice_modes.yaml`** must keep **strict** guidance: use **`<PS_PAYLOAD>` only when the user needs pasteable output** (draft, code, list, …), not for every short conversational reply — otherwise hands-free loops produce constant pastes and noisy blocks.
...
- Widget `_on_escape_global` stops on **`recorder.is_recording`** without requiring `status=="listening"` (race: `set_status` is `after(0)`). **Fallback (0.29.4):** if **`status == "listening"`** or **`orch._is_recording`** but the recorder flag already dropped, still call **`stop_recording()`** so Esc / Alt+Q cannot no-op in a desynced state. Avatar click uses the same fallback after handling Continuous-off and TTS stop.
...
**Settings → Performance — LLM sampling (Beta 0.29.4+):** Rows **`LLM temperature`**, **`LLM top-p`**, **`LLM top-k`**, **`LLM min-p`**, **`LLM repeat penalty`**, **`LLM presence penalty`**, **`LLM frequency penalty`** (**`widget.py`**; string keys `settings.performance.llm_*`) mirror **`llm.temperature`** through **`llm.frequency_penalty`** in **`Data/Configs/config.yaml`**. On save, **`_settings_change_flags`** treats sampling tuple changes like other LLM fields (requires restart path). **`Orchestrator._llm_sampling_kwargs()`** merges these from runtime config into LLM **`process(...)`** calls; **`LlamaCppServerLLM`** (`App/services/llm/llamacpp_server.py`) folds **`temperature`** plus optional **`top_p`**, **`top_k`**, **`min_p`**, **`repeat_penalty`**, **`presence_penalty`**, **`frequency_penalty`** into the JSON **`/v1/chat/completions`** body when present. Factory presets and rationale: **`App/configs/defaults.yaml`** comments under **`llm:`**.
...
## Skins — layout, IDs, resolution (Beta 0.29.0)
...
### Pro TTS — engines, PyTorch CUDA, voice ref, LLM prompt extension (0.28.3–0.29.0)
...
- **Runtime scope:** toggles that alter request payload (`chat_template_kwargs`, diagnostic JSON preview) apply immediately; changing **`--reasoning-budget`** is a server-start flag and requires llama-server restart.
...
- **Leak safety net:** **`Orchestrator._strip_thinking_blocks`** (static helper) strips **`<think>`**, **`<thinking>`**, **`<reasoning>`**, **`<redacted_thinking>`** (case-insensitive), including empty blocks, multiline blocks, unterminated openings, and orphan closing tags; applied to LLM output on main pipelines (Answer, Help, summaries, translations, greeting generation, etc.).
- **UI defense-in-depth:** Chat/Help assistant bubble rendering (`widget.py`) applies an extra residual tag strip before display, so rare backend leaks do not surface as visible HTML-like tags in bubbles.
- **Diagnostics:** Advanced LLM template diagnostic supports request bisection (`Omit chat_template_kwargs`) and explicit payload visibility (`Show JSON request body in results`) to confirm what is sent on the wire.
...
- **Architecture / folders:** Creates `Data\Tools\` backends: at least **`nvidia-cuda-12.4`**, **`nvidia-cuda-13.1`** (when used), **`vulkan`**, **`cpu`** as needed; **`install_llama_backend.ps1`** downloads/extracts **llama.cpp** release zips (see `LLAMA_VER` in `install.bat`, e.g. b8860).
...
- **Reasoning/thinking hardening:** `Orchestrator._strip_thinking_blocks` now handles `think|thinking|reasoning|redacted_thinking` leak variants, including empty, partial, and orphan-tag cases.
- **Chat/Help rendering guard:** assistant bubble display path now strips residual thinking tags as defense-in-depth before text is shown.
- **Greeting consistency:** Chat greeting cache stores only post-sanitization visible text; greeting auto-TTS uses that same sanitized text source.
- **Diagnostics clarity:** Advanced diagnostic explicitly supports request-level bisection (`Omit chat_template_kwargs`) vs CLI-level behavior (`--reasoning-budget` restart requirement).

### Beta 0.29.4 (April 2026) — shipped
- **LLM sampling UI + plumbing:** Seven Performance dropdowns for **`llm.temperature`**, **`top_p`**, **`top_k`**, **`min_p`**, **`repeat_penalty`**, **`presence_penalty`**, **`frequency_penalty`** — **`widget.py`** persistence → **`Orchestrator._llm_sampling_kwargs()`** → **`LlamaCppServerLLM`** OpenAI-compat payload (`_SAMPLING_PAYLOAD_KEYS`).
- **STT + `mix`:** `answer` and `transcribe` use **single-pass** mixed audio via early return in **`_build_dual_mix_raw_text`**; reduces missing syllables at end of utterance vs dual-segment path.
- **Continuous Chat:** worker gating + **`reset_continuous_chat_listen_blockers`** on user stop; idle low-energy skip relaxed for longer clips; **`_extend_continuous_tts_cooldown`** logging when **`pipeline_debug`**.
- **Manual stop:** **`stop_recording`** clears **`_is_recording`**; widget Esc/Alt+Q/avatar **Listening** fallback — `orchestrator.py` + `widget.py`.
- **Diagnostics:** **`audio.pipeline_debug`** + **`[pipeline]`** / **`[continuous]`** on **`perkysue`**; **`App/utils/audio.py`** mix stats (mic queue drops, track sample counts). **TROUBLESHOOTING.md** § debug.
- **OmniVoice/TTS stabilization:** Windows TorchCodec + FFmpeg shared installer path stabilized; OmniVoice post-download verification now supports generated audio returned as either `torch.Tensor` or `numpy.ndarray`, preventing false install failures at the final test-generate step.
- **Voice repair UX:** Voice-tab repair/full-repair sequence was hardened to avoid multi-instance/hotkey conflicts during `install.bat` relaunch and to keep recovery behavior explicit for non-technical users.
- **Plugin host connector (internal beta):** validated connector path for the optional Brainstorm extension through the `Data/Plugins/<id>/` SPI workflow (internal prototype for roundtable/debate-style AI conversations; not required for core runtime).

### Beta 0.29.3 (April 2026) — shipped
- **Fallback defaults fixed (existing installs):** `llm.answer_context_keep` fallback now resolves to **2** (not 4) and `llm.inject_all_modes_in_chat` fallback now resolves to **true** when keys are missing in `Data/Configs/config.yaml`.
- **Post-update dependency safety:** runtime consistency check now probes `webrtcvad` and conditionally `pyaudiowpatch` when Windows loopback modes (`system_only` / `mix`) are active, reducing silent fallback to mic-only after update.
- **Support logs:** post-update dependency probe now logs `capture_mode`, required module list, and a short failure excerpt for faster machine diagnostics.
...
- **Mix source-aware context (0.29.2):** dual-track capture + **`_build_dual_mix_raw_text`** in `App/utils/audio.py` + `App/orchestrator.py` can separate mic vs loopback for labelled STT. **0.29.4 follow-up:** modes **`answer`** and **`transcribe`** intentionally **skip** that path and use **single-pass** mixed audio for dictation fidelity (see § *Audio capture* above).
...
- **Docs / version:** `CHANGELOG.md`, README, this file, **`GETTING_STARTED.md`**, **`TROUBLESHOOTING.md`**, KB + `perkysue_kb.md`, `APP_VERSION` was **Beta 0.28.7** (superseded by 0.28.8, **0.28.9**, then **0.29.0**).
